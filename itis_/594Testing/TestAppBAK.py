...
test = cdll.LoadLibrary('/Library/Frameworks/icoreClient.framework/Versions/Current/iCoreClient')
...
def Test_TmccSetUpdateSetting(bNeedAutoUpdate, iRepeatInterval, iStartDay,iStartTime):
    test_Schedule_T = Test_Schedule_t()                        
    test_Schedule_T.RepeatInterval = iRepeatInterval
...
def Test_TmccGetUpdateSetting():
    ...
    ScheduleAU = [bNeedAutoUpdate, iRepeatInterval, iStartDay,iStartTime]
    return ScheduleAU
...
def Test_TmccSetProxySetting(PxyType, bUseSysPxy, bUsePxy, Proto, Server, Port, bNeedPWD, Username, Password):
    ...
    if bUsePxy ==1:
        ...
        test_TmccProxySetting_t.ServerAddress = None
...
class Test_TmccUpdateVer_T(Structure):
    _fields_=[("list",Test_TmccVerItem_T*TMCC_COMP_NUMBER)]
...
def Test_TmccGetVersion():
    ...
    #testlist=Test_TmccVerItem_T*TMCC_COMP_NUMBER
    Test_TmccUpdateVer_T
    ...
    #for index in range(0,TMCC_COMP_NUMBER):
#
#        ComponentTypes.append(tests[index].ComponentType)
#
#        ComponentVers.append(tests[index].ComponentVer)
#        #print index
#        #print tests[index].ComponentVer
#
#        UpdateTime.append(tests[index].Update_time)
#        
#    Contents=[ComponentTypes,ComponentVers,UpdateTime]
...
##def Test_TmccGetVersion():
##
##    print "in the GetVersion"
##    test_TmccUpdateVer_T = Test_TmccUpdateVer_T()
##    nRet=test.TmccGetVersion(pointer(test_TmccUpdateVer_T))
##    print "nRet=%d", nRet
##
##    print test_TmccUpdateVer_T
##    LNumber = test_TmccUpdateVer_T.ListNum
##
##    ComponentType = []
##    ComponentVer = []
##    Update_time = []
...
##    while i<LNumber:
##        ComponentType.append(test_TmccUpdateVer_T.List[i].ComponentType)
##        ComponentVer.append(test_TmccUpdateVer_T.List[i].ComponentVer)
##        Update_time.append(test_TmccUpdateVer_T.List[i].Update_time)
##        ComponentList = [ComponentType, ComponentVer,Update_time]
##        print ComponentList
...
##
##def Test_TmccCallback_t(cmd, pNotifyData):
##    f = open("testNotify.txt", 'a')
##    
##    f.write(time.ctime() + " ~~~ " +"CMD: " + str(cmd)  + "\n")
##    f.write(time.ctime() + " ~~~ " +"NotifySize: " + str(pNotifyData[0].size) + "\n")
##
##
##    if cmd == 10:
##    # or cmd =='TMCME_WP_RESULT'
##                  
##    
##        f.write(time.ctime() + " ~~~ " + " WTP Notify for szURL: "+ str(pNotifyData[0].Data[0].WPNotify.szURL) + "\n")
##        f.write(time.ctime() + " ~~~ " +"NWTP Notify for ScanType: "+ str(pNotifyData[0].Data[0].WPNotify.ScanType) + "\n")
##              
##        if pNotifyData[0].Data[0].WPNotify.ScanType == 1:
##            f.write(time.ctime() + " ~~~ " +"WTP Notify for szThreat: "+ str(pNotifyData[0].Data[0].WPNotify.ScanResult.WTP.szThreat) + "\n")
##            f.write(time.ctime() + " ~~~ " +"WTP Notify for Threat: "+ str(pNotifyData[0].Data[0].WPNotify.ScanResult.WTP.Threat) + "\n")
##
##        else:
##            f.write(time.ctime() + " ~~~ " +"CategoryCode: "+ str(pNotifyData[0].Data[0].WPNotify.ScanResult.Category.CategoryCode) + "\n")
##            f.write(time.ctime() + " ~~~ " +"CategoryName: "+ str(pNotifyData[0].Data[0].WPNotify.ScanResult.Category.wszCategoryName) + "\n")
##
##           
##    f.write("\n\n\n")
##    
...
##
##
##Test_CallbackNotify = CFUNCTYPE(c_int, c_int, POINTER(Test_NotifyData))
##
##Test_CBackNotify = Test_CallbackNotify(Test_TmccCallback_t)
##
##test.TmccRegisterListener(Test_CBackNotify)



####just for init and release lib###########
